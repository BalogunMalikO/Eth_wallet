...
use web3::{
    transports, types::{Address, U256, TransactionParameters,},Web3,
};
...
impl Wallet {
    pub fn new(secret_key: &SecretKey, public_key: &PublicKey) -> Self{
        let addr = public_key_addy(&public_key);
        Wallet {
    ...
    pub fn Read_from_file(file_path: &str) -> Result<(Wallet)> {
...
pub fn public_key_addy (public_key: &PublicKey) -> Address {
...
pub async fn sign_and_send(
    ...
    let signed = web3.accounts().sign_transaction(transaction, &secret_key)
        .await?;
    let transaction_result = web3.eth().send_raw_transaction(signed.raw_transaction)
        .await?;
